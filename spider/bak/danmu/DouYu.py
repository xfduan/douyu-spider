...
class DouYuDanMuClient(AbstractDanMuClient):
    ...
    def _prepare_env(self):
        return ('119.96.201.28', 8601), {'room_id': self.roomId}
    ...
    def _create_thread_fn(self, roomInfo):
        def keep_alive(_self):
            _self.danmuSocket.push('type@=keeplive/tick@=%s/' % int(time.time()))
            time.sleep(30)
        ...
        def get_danmu(_self):
            ...
            for msg in re.findall(b'(type@=.*?)\x00', content):
                try:
                    ...
                    _self.danmuWaitTime = time.time() + _self.maxNoDanMuWait
                    _self.msgPipe.append(msg)
